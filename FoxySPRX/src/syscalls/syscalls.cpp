...
{
    system_call_2(SYS_STORAGE_GET_DEVICE_INFO, device, (uint64_t)device_info);
    return_to_user_prog(int);
}
...
{
    system_call_7(SYS_STORAGE_READ, fd, 0, start_sector, sectors, (uint64_t)bounce_buf, (uint64_t)sectors_read, 0);
    return_to_user_prog(int);
}
...
{
    system_call_7(SYS_STORAGE_READ, fd, 0, start_sector, sectors, (uint64_t)bounce_buf, (uint64_t)sectors_read, flags);
    return_to_user_prog(int);
}

int sys_storage_write(int fd, uint64_t start_sector, uint64_t sectors, uint8_t* buf, uint32_t* sectors_written, uint64_t flags)
{
    system_call_7(SYS_STORAGE_WRITE, fd, 0, start_sector, sectors, (uint64_t)buf, (uint64_t)sectors_written, flags);
    return_to_user_prog(int);
}
...
{
    system_call_3(_SYS_PRX_UNLOAD_MODULE, (uint64_t)id, (uint64_t)flags, (uint64_t)pOpt);
    return_to_user_prog(int);
}
...
{
    system_call_6(_SYS_PRX_STOP_MODULE, (uint64_t)id, (uint64_t)args, (uint64_t)argp, (uint64_t)modres, (uint64_t)flags, (uint64_t)pOpt);
    return_to_user_prog(int);
}
...
{
    system_call_3(_SYS_PRX_GET_MODULE_ID_BY_NAME, (uint64_t)name, (uint64_t)flags, (uint64_t)pOpt);
    return_to_user_prog(sys_prx_id_t);
}

sys_prx_id_t _sys_prx_get_module_id_by_address(void* addr)
{
    system_call_1(_SYS_PRX_GET_MODULE_ID_BY_ADDRESS, (uint64_t)addr);
    return_to_user_prog(sys_prx_id_t);
}
