...
extern int sys_map_path(char** paths, char** new_paths, int num);
extern int cobra_load_vsh_plugin(unsigned int slot, char* path, void* arg, uint32_t arg_size);
extern int cobra_unload_vsh_plugin(unsigned int slot);
extern int ps3mapi_get_core_version();
extern int ps3mapi_get_core_minversion();
extern int ps3mapi_get_fw_type(char* fw);
extern int ps3mapi_get_fw_version();
extern int ps3mapi_get_all_process_modules_prx_id(sys_pid_t pid, sys_prx_id_t* prx_id_list);
extern int ps3mapi_get_process_module_name_by_prx_id(sys_pid_t pid, sys_prx_id_t prx_id, char* name);
extern int ps3mapi_get_process_module_filename_by_prx_id(sys_pid_t pid, sys_prx_id_t prx_id, char* filename);
extern int ps3mapi_load_process_modules(sys_pid_t pid, char* path, void* arg, uint32_t arg_size);
extern int ps3mapi_unload_process_modules(sys_pid_t pid, sys_prx_id_t prx_id);
extern int ps3mapi_unload_vsh_plugin(char* name);
extern int ps3mapi_get_vsh_plugin_info(unsigned int slot, char* name, char* filename);
extern int ps3mapi_load_kernel_plugin(uint8_t* payload, int size);
extern int ps3mapi_load_dynamic_kernel_plugin(uint8_t* payload, int size, uint64_t* residence);
extern int ps3mapi_unload_dynamic_kernel_plugin(uint64_t residence);
extern int ps3mapi_get_is_hen();
extern int ps3mapi_get_hen_rev();
extern int ps3mapi_poke_hen(uint64_t address, uint64_t value);
extern int ps3mapi_process_page_allocate(sys_pid_t pid, uint64_t size, uint64_t page_size, uint64_t flags, uint64_t is_executable, uint64_t* page_address);
extern int ps3mapi_get_process_module_segments(sys_pid_t pid, sys_prx_id_t prx_id, sys_prx_module_info_t* info);
extern int ps3mapi_create_process_thread(sys_pid_t pid, thread_t* thread, void* entry, uint64_t arg, int prio, size_t stacksize, const char* threadname);
extern int ps3mapi_get_all_processes_pid(uint32_t* pid_list);
extern int ps3mapi_get_process_name_by_pid(uint32_t pid, char* pid_name);
extern int ps3mapi_get_process_by_pid(uint32_t pid, process_t process);
extern int ps3mapi_get_current_process_critical(process_t process);
extern int ps3mapi_get_current_process(process_t process);
extern int ps3mapi_check_syscall(int num);
extern int ps3mapi_disable_syscall(int num);
extern int ps3mapi_pdisable_syscall8(int mode);
extern int ps3mapi_pcheck_syscall8();
extern int ps3mapi_get_idps(uint64_t* idps);
extern int ps3mapi_set_idps(uint64_t part1, uint64_t part2);
extern int ps3mapi_get_psid(uint64_t* psid);
extern int ps3mapi_set_psid(uint64_t part1, uint64_t part2);
extern int ps3mapi_remove_hook(void);
extern int ps3mapi_enable_access_syscall8(uint64_t key);
extern int ps3mapi_disable_access_syscall8(uint64_t key);
extern int sys_get_mamba();
extern int sys_get_version(uint32_t* version);
extern int sys_get_version2(uint16_t* version);
extern uint64_t ps3mapi_lv1_peek(uint64_t addr);
extern uint64_t ps3mapi_lv1_poke(uint64_t addr, uint64_t value);
extern uint64_t ps3mapi_lv2_peek(uint64_t addr);
extern uint64_t ps3mapi_lv2_poke(uint64_t addr, uint64_t value);
extern int PS3MAPIGetMemory(sys_pid_t pid, void* destination, void* source, size_t size);
extern int PS3MAPISetMemory(sys_pid_t pid, void* destination, const void* source, size_t size);
extern uint64_t PeekLv1(uint64_t addr);
extern void PokeLv1(uint64_t addr, uint64_t value);
extern uint64_t PeekLv2(uint64_t address);
extern void PokeLv2(uint64_t addr, uint64_t val);
extern bool PeekChunkLV1(uint64_t start, uint64_t* buffer, uint64_t size);
extern bool PeekChunkLV2(uint64_t start, uint64_t* buffer, uint64_t size);
// lv2_lv1_call - rebug // r10 is the lv1 syscall number. The registers r3 - r9 are arguments.
extern uint64_t lv2_lv1_call(uint64_t syscallnum, uint64_t arg1, uint64_t arg2, uint64_t arg3, uint64_t arg4, uint64_t arg5, uint64_t arg6, uint64_t arg7);
...
extern uint64_t lv2_func_call(uint64_t addr, uint64_t arg1, uint64_t arg2, uint64_t arg3, uint64_t arg4, uint64_t arg5, uint64_t arg6, uint64_t arg7);
extern int sys_dbg_read_process_memory(sys_pid_t pid, void* destination, void* source, size_t size);
extern int sys_dbg_write_process_memory(sys_pid_t pid, void* destination, const void* source, size_t size);
extern int sys_storage_get_device_info(uint64_t device, device_info_t* device_info);
extern int sys_storage_open(uint64_t id, int* fd);
extern int sys_storage_close(int fd);
extern int sys_storage_read(int fd, uint32_t start_sector, uint32_t sectors, uint8_t* bounce_buf, uint32_t* sectors_read);
extern int sys_storage_read_with_flags(int fd, uint32_t start_sector, uint32_t sectors, uint8_t* bounce_buf, uint32_t* sectors_read, uint32_t flags);
extern int sys_storage_write(int fd, uint64_t start_sector, uint64_t sectors, uint8_t* buf, uint32_t* sectors_written, uint64_t flags);
extern int sys_storage_get_cache_of_flash_ext_flag(uint8_t* flag);
extern bool is_nor();
extern int lv2_get_platform_info(platform_info_t* info);
extern int get_target_type(uint64_t* type); // 1-CEX, 2-DEX, 3-DECR/RefTool
extern bool IsConsoleCex();
extern bool IsConsoleDex();
extern bool IsConsoleDeh();
extern bool IsConsoleHen();
extern bool IsConsoleMamba();
extern bool IsConsoleCobra();
extern uint16_t GetPayloadVersion();
extern bool DoesConsoleHaveCCAPI();
extern bool DoesConsoleHaveWebman();
extern int update_mgr_write_eprom(uint64_t flag_offset, uint64_t value);
extern int update_mgr_read_eprom(uint64_t* flag_offset, uint64_t value);
extern int set_recovery_mode();
extern int sys_net_eurus_etherAddr(uint8_t* etherAddr);
extern int sys_ss_appliance_info_manager(uint32_t packet_id, uint8_t* out);
extern void get_idps_info_manager();
extern int GetGameID(char titleId[]);
extern int ps3_led(uint64_t color, uint64_t mode);
extern int ring_buzzer_simple(void);
extern int ring_buzzer_double(void);
extern int ring_buzzer_triple(void);
extern int sys_fs_chown(const char* path, int uid, int gid);
extern int sys_fs_mount(const char* device_path, const char* device_name, const char* file_system);
extern int sys_fs_unmount(const char* device_path);
extern void del_turnoff(uint8_t beeps);
extern int ps3_shutdown(void);
extern int ps3_hard_restart(void);
extern int ps3_quick_restart(void);
extern int ps3_soft_restart(void);
extern void ps3_exit_xmb();
extern void enable_dev_blind();
extern void disable_dev_blind();
extern int sys_dbg_get_thread_list(sys_pid_t pid, sys_ppu_thread_t* ids, uint64_t* ids_num, uint64_t* all_ids_num);
extern int sys_game_get_temperature(uint32_t _dev, uint32_t* temperature);
extern int get_temperature_celcius(uint32_t& cpu_temp, uint32_t& rsx_temp);
extern int sys_sm_set_fan_policy(uint8_t arg0, uint8_t arg1, uint8_t arg2);
extern int sys_sm_get_fan_policy(uint8_t id, uint8_t* st, uint8_t* mode, uint8_t* speed, uint8_t* unknown);
extern void ps3_set_recovery_mode();
extern int _sys_prx_unload_module(sys_prx_id_t id, sys_prx_flags_t flags, const sys_prx_unload_module_option_t* pOpt);
extern int _sys_prx_stop_module(sys_prx_id_t id, size_t args, void* argp, int* modres, sys_prx_flags_t flags, sys_prx_stop_module_option_t* pOpt);
extern sys_prx_id_t _sys_prx_get_module_id_by_name(const char* name, sys_prx_flags_t flags, sys_prx_get_module_id_by_name_option_t* pOpt);
extern sys_prx_id_t _sys_prx_get_module_id_by_address(void* addr);
extern sys_prx_id_t _sys_prx_get_my_module_id();
// prxloader = mandatory; cobra = optional; ccapi = don't use !!!
extern void _sys_ppu_thread_exit(uint64_t val);